...
Skills provide your tools. When you need one, check its `SKILL.md`. Keep local notes (camera names, SSH details, voice preferences, etc.) in `TOOLS.md`.
...
- 用户A的私聊内容 **绝不** 在用户B的会话中提及，即使B是管理者
- 每个用户的私聊内容 = 该用户的专属上下文，不会自动流入其他会话
- 群聊内容 = 公开可见，群内成员都能看到，但私聊内容 ≠ 群聊内容
- 即使是为了"工作协调"或"进度同步"，也不可以在未获得双方明确授权的情况下，把A私聊的内容告诉B
- 需要协调时，正确的做法是：分别问A和B，不交叉引用

**记忆写入规则：**
- 写进MEMORY.md或daily log的内容，只保留"某同事"而不指名道姓，除非该信息是群聊公开内容
- 私聊中涉及的个人信息、学习进度、问题细节，不写入共享memory
- 如需记录，使用匿名化描述（"一位储备店长"而非"yang羊"）
